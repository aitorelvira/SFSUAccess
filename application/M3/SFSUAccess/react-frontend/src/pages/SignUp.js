...
const SignUp = () => {
  ...
  const register = ()=>{
    // get user inputs 
    let first_name = document.getElementById("first_name").value.trim().toLowerCase();
    let last_name =  document.getElementById("last_name").value.trim().toLowerCase();
      ...
      if(email.endsWith("@mail.sfsu.edu")){
        ...
        .then((response) =>{
          if(response.data){
            ...
            setCountdown("Redirecting to sign in page...");
          }
          else
  ...
  return (
    <div>
     ...
     <Container className="overAll">
        ...
        <div className="message">{message}<br/>{countdown}</div><br/>
        <Form>
